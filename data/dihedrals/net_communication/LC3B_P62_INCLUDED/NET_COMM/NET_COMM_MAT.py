...
def net_comm(structure,MI_mat,source,sink):
    ...
    LC3B_II_SOURCE_vicinity=LC3B_II.select_atoms('not resid 120 and around 3 resid %d'%source)
    LC3B_II_TARGET_vicinity=LC3B_II.select_atoms('not resid 120 and around 3 resid %d'%sink)
    ...
    resid_all=LC3B_II.select_atoms('not resid 120') #all residues including p62
...
arr = [i for i in range(1,120)]+[i for i in range(338,345)] #including p62 residues
r = 2
comb=rSubset(arr, r)
...
for i in tqdm.tqdm(comb):
    net_comm_mat[arr.index(i[0]),arr.index(i[1])]=net_comm(structure,MI_mat,i[0],i[1])
